...
def get_grid_coords(pts, bounds):
    '''
        pts:       1 x n x 3
        bounds:    1 x n x 2 x 3
    '''
    pts = pts[0]
    bounds = bounds[0]
    min_xyz = bounds[:, 0]
    max_xyz = bounds[:, 1]
    bounds = max_xyz - min_xyz
    grid_coords = (pts - min_xyz) / bounds
    grid_coords = grid_coords * 2 - 1
    return grid_coords
...
def bilinear_sample_triplanes(points, triplanes):
    '''
    Inputs:
    
    points: (n, 3) bbox坐标系内的点, 大部分分布在[-1, 1]
    
    Returns:
    
    xyz_f: (n, 3 * channel)'''
    n, _ = points.shape
    feat_xy, feat_yz, feat_xz = triplanes[None].chunk(3, dim=1)
    ...
    xyz_f = torch.cat((xy_f, yz_f, xz_f), dim=1).reshape(n, channel * 3)
